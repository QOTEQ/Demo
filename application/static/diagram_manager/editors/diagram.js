...
const diagram = {
  ...
  flow:null,
    ...
    init(element){
          ...
          paper.on('group:toggle:button:pointerdown', this.groupToggleButtonPointerdownHandler.bind(this));
  ...
  //...........UPDATE ELEMENTS AND LINKS
    ...
    updateGraph(flow){
      ...
      this.paper.freeze();
      this.graph.clear();
      let cells = [], main, previous, node, element, link, pair;
      ...
      for (let step of flow[0].body) {
        ...
        for (let type of types) {
          ...
          if (type == 'success') {
            pair = this.createElementAndLink('main', {command:step.command, embeds:this.findEmbeds(flow, step.command)}, previous);
            main = pair.element;
            cells.push(pair.element);
            if (pair.link) cells.push(pair.link);
          }
...
//.......PAN
    ...
    blankPointermoveHandler(evt, x, y){
        ...
        if (this.origin.x && this.origin.y && (x != this.origin.x || y != this.origin.y)) {
