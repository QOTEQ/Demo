...
class codeEditor{
  ...
  constructor(id, modules, config = {}) {
      ...
      document.addEventListener('selectionchange', (event) => {
          if (document.activeElement === this.elements.codeEditorInput) this.onSelectionChange(document.activeElement.selectionStart, document.activeElement.selectionEnd);
      });
      this.elements.codeEditorInput.addEventListener('input', (event)=>this.updateValue(event.target.value));
      ...
      this.elements.codeEditorInput.addEventListener('keydown', this.onEditorKeydown.bind(this));
  ...
  //  else this.elements.processSaveButton.removeAttribute('disabled');
    ...
    this.modules.events.emit(this.id + ':input:change', {text, lines, isOriginal, changedLines});
  ...
  onEditorKeydown(e){
      ...
      if (keyCode === 9) {  // TAB = 9
          e.preventDefault();
          this.elements.codeEditorInput.value = value.slice(0, selectionStart) + '\t' + value.slice(selectionEnd);
          this.elements.codeEditorInput.setSelectionRange(selectionStart+1, selectionStart+1)
      }
      else if (keyCode === 40) {
  ...
  showAutocomplete(show = true){
      ...
      if (show) {
          ...
          let top = 10 + (this.editingLine + 1) * 19 + 2 - this.elements.codeEditorInput.scrollTop;
          const codeEdotorRect = this.elements.codeEditorInput.getBoundingClientRect();
  ...
  updateAutocomplete(event){
      ...
      const lines = this.elements.codeEditorInput.value.substr(0, this.elements.codeEditorInput.selectionStart).split("\n")
