...
class flowDiagram extends baseDiagram {
    ...
    constructor(id, modules){
        ...
        this.paper.on('group:toggle:button:pointerdown', this.groupToggleButtonPointerdownHandler.bind(this));
  ...
  //...........UPDATE ELEMENTS AND LINKS
    ...
    updateGraph(flow){
      ...
      this.paper.freeze();
      this.graph.clear();
      let cells = [], step, main, previous, node, element, link, pair;
      ...
      step = flow[0].body[0]
      previous = this.createElement('main', {command:step.command, embeds:this.findEmbeds(flow, step.command)})
      cells.push(previous);
      ...
      for (let i=0; i<flow[0].body.length; i++) {
        ...
        for (let type of types) {
          ...
          for (let command of step[type]) {
              pair = this.createElementAndLink(type, {command, embeds:this.findEmbeds(flow, command)}, previous);
              cells.push(pair.element);
              if (pair.link) cells.push(pair.link);
          }
          if (type == 'success' && nextStep) {
            pair = this.createElementAndLink('main', {command:nextStep.command, embeds:this.findEmbeds(flow, nextStep.command)}, previous);
            main = pair.element;
            cells.push(pair.element);
            if (pair.link) cells.push(pair.link);
          }
  ...
  createElement(type, node){
    ...
    const colors = {main:'#2f76fe', success:'#cdcdcd', fail:'#e6c456', finalization:'#e6c456'};
  ...
  createLink(type, source, target){
      const colors = {main:'#2f76fe', success:'#00e60a', fail:'#ff1a00', finalization:'#ff1a00'};
  ...
  groupToggleButtonPointerdownHandler(elementView){
    ...
    if (data && data.embeds) {
      ...
      this.modules.dialogs.alert('<ol class="flow-diagram-embeds-list">' + data.embeds.map(e=>`<li>${e.command.replaceAll('`', '')}</li>`).join('') + '</ol>', {title:data.command, buttons:{ok:true} });
    }
