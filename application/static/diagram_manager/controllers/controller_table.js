...
const controllerTable = {
  ...
  async select(name){
    ...
    this.elements.entityTableBody.innerHTML = '';
    const data = await this.modules.transport.send(method, {[id]: '>0'}, true);
    ...
    const header = '<tr>' + Object.keys(data.result[0]).map(key => `<th>${key}</th>`).join('') + '</tr>';
     this.elements.entityTableHeader.innerHTML = header;
     const body = data.result.map(row => `<tr>${Object.values(row).map(value => `<td>${value}</td>`).join('')}</tr>`).join('');
