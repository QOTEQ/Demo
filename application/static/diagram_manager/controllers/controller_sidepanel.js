...
class controllerSidepanel {
  ...
  constructor(id, modules) {
      ...
      // console.log($)
      $(function () { $('#jstree').jstree(treedata); });
  ...
  selectTreeItem(e, data) {
    ...
    const id = data.selected[0], parent = data.node.parent, parents = data.node.parents, ext = id.split('.').pop();
    // console.log(data, ext);
    const views = {'main':'Main', 'js':'Javascript', 'md':'Markdown', 'sql':'Sql', 'css':'Css', 'html':'Html', 'json':'Json', 'flow':'Diagram', 'pgsql':'Table'}
