...
const descriptions = {
    ...
    class:{
        ...
        toggleSize: function(collapsed){
            ...
            if (el) {
                ...
                if (header) {
                    ...
                    width = Math.round(44 + tspan.getBoundingClientRect().width + 10 + buttonSize + (headerHeight - buttonSize) / 2);
        ...
        toggleEmbeddedLinks: function(collapsed){
            ...
            if (collapsed) {
                this.links = {};
                let inbound = this.graph.getConnectedLinks(this, {inbound:true, deep:true});
                let outbound = this.graph.getConnectedLinks(this, {outbound:true, deep:true});
                for (let l of inbound) {
                    // if (l.prop('parent') == this.prop('id')) continue;
                    this.links[l.id] = {source:l.prop('source'), target:l.prop('target')};
            ...
            if (collapsed) {
                ...
                if (this.lastPosition){
                    ...
                    if (pos.x != this.lastPosition.x || pos.y != this.lastPosition.y) {
                        offset = {x:pos.x - this.lastPosition.x, y:pos.y - this.lastPosition.y};
                ...
                for (let cell of this.children) {
                    if (offset) {
                        ...
                        if (pos) cell.prop('position', {x:pos.x + offset.x, y:pos.y + offset.y});
                    }
                    this.embed(cell);
                }
                this.children = [];
 ...
 if (!joint.shapes.html.GroupWithHeader) {
    joint.shapes.html.GroupWithHeader = joint.dia.Element.define('html.GroupWithHeader', descriptions.instance, descriptions.class, descriptions.static);
 }
...
const GroupWithHeader = {
    ...
    newElement(config){
        ...
        if (config.ports) {
            if (config.ports.groups) cell.prop(['ports', 'groups'], config.ports.groups);
    ...
    updateCell(config, cell){
        if (config.icon) cell.attr(['icon', 'xlinkHref'], config.icon.includes('/') ? config.icon : '/img/diagrams/' + config.icon);
        if (config.header) {
            if (config.header.text) cell.attr(['headerText', 'text'], config.header.text);
